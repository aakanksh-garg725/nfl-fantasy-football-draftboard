-- Replaces the free-entry "number of rounds" field on draft creation with
-- explicit roster construction (starting slots per position + bench) and a
-- scoring format. round_count is now derived (sum of all roster slots)
-- rather than chosen directly. Roster counts are informational/derived only
-- — nothing in make_pick enforces position limits, so drafters remain free
-- to fill any slot with any position (e.g. an extra WR instead of a DST).

alter table public.drafts drop constraint if exists drafts_round_count_check;

alter table public.drafts
  add column scoring_format text not null default 'ppr'
    check (scoring_format in ('ppr', 'half_ppr', 'non_ppr')),
  add column roster_qb int not null default 1 check (roster_qb between 0 and 4),
  add column roster_rb int not null default 2 check (roster_rb between 0 and 6),
  add column roster_wr int not null default 2 check (roster_wr between 0 and 6),
  add column roster_te int not null default 1 check (roster_te between 0 and 4),
  add column roster_flex_rb_wr int not null default 1 check (roster_flex_rb_wr between 0 and 4),
  add column roster_flex_wr_rb_te int not null default 1 check (roster_flex_wr_rb_te between 0 and 4),
  add column roster_superflex int not null default 0 check (roster_superflex between 0 and 4),
  add column roster_k int not null default 1 check (roster_k between 0 and 2),
  add column roster_dst int not null default 1 check (roster_dst between 0 and 2),
  add column roster_bench int not null default 6 check (roster_bench between 4 and 8);
